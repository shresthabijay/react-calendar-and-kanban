...
export const dateRangeForMonth=(date)=>{
    ...
    let dateToIncludeAtFront= moment(date).subtract(1,"months").endOf("month").subtract(firstDayOfMonth-1,"days")
    let lastDayOfMonth= endOfMonth.clone().day()
    let dateToIncludeAtBack=moment(date).add(1,"months").startOf("month").add(6-lastDayOfMonth,"days")
...
export const findFirstSunday=(date)=>{
    let firstSundayOfTheMonth=date.clone().startOf("months").day(0); 
